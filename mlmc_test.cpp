...
void mlmc_test(void (*mlmc_l)(int, int, double *, std::mt19937& _rng), int M,int N,int L,
               int N0, float *Eps, int Lmin, int Lmax, FILE *fp, std::mt19937& _rng) {
...
//
  ...
  while (Eps[i]>0) {
    ...
    float P = mlmc(Lmin,Lmax,N0,eps,mlmc_l, alpha,beta,gamma, Nl,Cl, _rng);
